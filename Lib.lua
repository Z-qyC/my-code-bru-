    ...
    local mfloor, mceil, mclamp, mmax, mmin, mabs, msin, mcos, mtan, mrad, mround = math.floor, math.ceil, math.clamp, math.max, math.min, math.abs, math.sin, math.cos, math.tan, math.rad, math.round
    local tinsert, tremove, tfind, tconcat, tunpack = table.insert, table.remove, table.find, table.concat, table.unpack
    local sformat, sgmatch, sgsub = string.format, string.gmatch, string.gsub
    getgenv().Library = {
        Directory = "octo",
        Folders = {
            "/fonts",
    ...
    local Fonts = {}; do
        ...
        function Library:ConvertHex(color)
            return sformat("#%02X%02X%02X", mfloor(color.R * 255), mfloor(color.G * 255), mfloor(color.B * 255))
        end
        function Library:ConvertFromHex(color)
            local c = sgsub(color, "#", "")
            return Color3.new(tonumber(c:sub(1, 2), 16) / 255, tonumber(c:sub(3, 4), 16) / 255, tonumber(c:sub(5, 6), 16) / 255)
        end
        ...
        function Library:Keypicker(properties)
            ...
            function Cfg.Set(color, alpha)
                ...
                if Items.Colorpicker.Visible then
                    ...
                    local Color = Items.MainColor.BackgroundColor3
            ...
            local Items = Cfg.Items; do
                ...
                Library:Create( "UIPadding", {
                    ...
                    Items.Text = Library:Create( "TextLabel" , {
                        ...
                        TextStrokeColor3 = rgb(255, 255, 255);
                        Text = 'AXIOM.IIV.xyz <font color = "rgb(235, 235, 235)">@placeholder / UID @ / Developer / 00/00/0000 / 00:00:00 / 0fps / Oms</font>';
                        Name = "\0";
                        AutomaticSize = Enum.AutomaticSize.XY;
                        BorderSizePixel = 0;
                        BorderColor3 = rgb(0, 0, 0);
        ...
        function Library:Notification(properties)
            local Cfg = {
                ...
                Type = properties.Type or "Normal",
                Items = {}
            }
        ...
        function Library:AddTab(properties)
            local Cfg = {
                ...
                Scale = properties.Scale or 1;
                ModelOffset = properties.ModelOffset or 1;
                VisualizedModel;
                Options = {};
                Chams = properties.Chams or false;
                ReturnedOptions = {};
            }
            local Items = Cfg.Items; do
                    Items.Outline = Library:Create( "TextButton" , {
                ...
                local Elements = Config.Items; do
                        ...
                        Elements.Options.Type = Section:Dropdown({
                            ...
                            Callback = function(option)
                                ...
                                task.wait()
                                Items.ViewportFrame.AutomaticSize = Enum.AutomaticSize.None
                                Elements.BoxHolder.Size = dim2(1, 0, 0, 0)
                                task.wait()
                                Elements.BoxHolder.Size = dim2(1, -2, 1, -2)
